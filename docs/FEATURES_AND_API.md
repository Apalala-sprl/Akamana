# Akamana — Features, API & Change Log

This document describes the Akamana feature set, the full HTTP API surface, the data model,
...
Akamana is an internal‑lab **certificate & key lifecycle service**. It runs as a single Rust
...
- **Second factor:** when the account has TOTP enabled (or `REQUIRE_MFA=true` and it has
  none), `/auth/login` answers with an MFA challenge instead of a session; the client
  finishes at `/auth/login/mfa` with a 6‑digit code or a recovery code. **Passkeys**
  (WebAuthn, opt‑in via `WEBAUTHN_RP_ID`/`WEBAUTHN_ORIGIN`) replace the password entirely.
- **Password reset:** `/auth/password-reset/request` mails a single‑use, 60‑minute link
  (needs SMTP and an email on the account); admins can also mint a link from the Users page.
- **Lockout:** 8 failed attempts for a username or source IP within 15 minutes → 429.
- All `/api/v1/*` endpoints require `Authorization: Bearer <token>` except the public allow‑list:
  `/health`, everything under `/api/v1/auth/`, `GET /api/v1/branding`,
  `GET /api/v1/certificates/root`, and `GET /api/v1/certificates/root/download/:platform`.
...
- Background scanner connects via TLS (using the per‑row SNI override), records expiry/subject/
  ...
  performs a **reachability cross‑check** (don't alert if Akamana's own network is down).

### 2.3 Network discovery
- `POST /api/v1/network/scan` probes a private **/24** (default = the Akamana server's own network),
...
- **Credentials** (SSH key/password, API token) — secrets **encrypted at rest**, never returned;
  can reuse an existing Akamana‑generated SSH key.
...
| GET | `/api/v1/branding` | *(public)* app title + which sign‑in options exist |
| POST | `/api/v1/auth/login` | *(public)* obtain JWT, or an MFA challenge |
| POST | `/api/v1/auth/login/mfa` · `/login/mfa/enroll/start` · `/login/mfa/enroll/finish` | *(public)* second‑factor step |
| POST | `/api/v1/auth/passkey/login/start` · `/finish` | *(public)* WebAuthn sign‑in |
| POST | `/api/v1/auth/password-reset/request` · `/confirm` | *(public)* self‑service reset |
| GET | `/api/v1/mfa/status` | own TOTP / passkey / recovery‑code state |
| POST | `/api/v1/mfa/totp/setup` · `/confirm` · `/disable` · `/api/v1/mfa/recovery-codes` | manage TOTP |
| GET/POST/DELETE | `/api/v1/mfa/passkeys` · `/passkeys/finish` · `/passkeys/:id` | manage passkeys |
...
| Method | Path | Purpose |
|---|---|---|
| GET/POST | `/api/v1/users` · PATCH `/:id/role` · PATCH `/:id/email` · POST `/:id/password` · POST `/:id/reset-link` · DELETE `/:id` | user admin (full_admin) |
...
- ~~**SAN support**~~ — **done** (migration 017): leaf certs carry DNS + IP SubjectAltNames
  (`sans_json`; CN is used as fallback SAN when none given).
- ~~**mTLS / EKU**~~ — **done** (migration 017): certificate purpose server / client / both →
  Extended Key Usage `serverAuth`/`clientAuth` (`eku_purpose`).
- **Monitoring table enhancements** — dedicated TLS‑version column, full cipher/protocol enumeration
  in details, and an issues‑only filter (partial: `tls_support_json` probe exists since migration 016).
- **SSH host‑key pinning** — the deployment client **and** the SFTP backup client currently accept
  any host key (lab‑acceptable; pin before wider use).
- **MFA under OIDC** — TOTP and passkeys are properties of the local account table, so in
  `AUTH_MODE=oidc` the identity provider owns the second factor; the Profile security
  section says so and stands down.

## 7. Added since the 2026‑05 expansion (migrations 016–021)

- **SAN + EKU** on issued leafs (017) — see above.
- **TLS protocol‑support probe** on monitored ports (016, `tls_support_json`).
- **Scoped API tokens** `ezk_…` (018) + API Tokens page and `require_human` gating.
- **SSH CAs & OpenSSH certificates** (019): auto‑bootstrapped User/Host CAs, signing,
  rotation, revocation — endpoints under `/api/v1/ssh/*`.
- **Encrypted backups & off‑box publishing** (020): scheduled/on‑demand DB backups
  (`.ezbak` passphrase/envelope encryption, recipients), remote path/SFTP destinations,
  **CRL publishing** (`/crl/<root_id>.crl` + remote push) and a publishable **root‑CA
  install HTML page** — see `docs/backup_and_restore.md`.
- **Expected certificate format** per application (021, `expected_cert_format`).
