...
WORKDIR /app
COPY --from=builder /src/backend/target/release/akamana-backend /app/akamana-backend
COPY web /app/web
COPY addons /app/addons
COPY deploy/nginx/akamana.conf /etc/nginx/conf.d/default.conf
COPY scripts/entrypoint-akamana.sh /entrypoint-akamana.sh
EXPOSE 8081
CMD ["/entrypoint-akamana.sh"]
